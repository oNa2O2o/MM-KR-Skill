...
#### 1.2 视频脚本（×3场景）
- 情节描述
- `{profile.language}` 台词原文（直接可表演，禁止内心戏）
- 中文翻译
- 结尾动作

#### 1.3 标题与标签
- 视频大标题备选×3（`{profile.language}` + 中文）
- 每幕小标题备选×3（`{profile.language}` + 中文）
- 标签（`{profile.language}`）
...
├── 视频1_角色A/
│   ├── XX线_完整文档.md
│   ├── XX线_DeepWhite编剧版.md（如用户选用）
│   ├── 分镜图/
│   └── Seedance参考/
│       ├── XX_Seedance提示词.md（三版并列）
│       ├── 场景A_图1_人设.png
│       └── 场景A_图2_分镜.png
├── 视频2_角色B/（同上）
└── 视频3_角色C/（同上）
...
### 画风
由 `{profile.art_style}` + `{profile.aesthetic_keywords}` 决定，不硬编码特定画风。
